...
export class SqlFollowRepository implements FollowRepository {
    ...
    VALUES (:id_follow, :followed_by, :user_id, :created_at, :updated_at)`,
      {
        ...
        followed_by: followModel.id_follow,
  ...
  async getById(id: string): Promise<Follow> {
    const followModel = await this.knex.raw<FollowModel[][]>(`SELECT * FROM follows WHERE id_follow = :id_follow`, {
      id_follow: id
    })
  ...
  async getFollowedIds(followerId: string): Promise<string[]> {
    const followModel = await this.knex.raw(`SELECT * FROM follow WHERE followed_by = :followed_by`, {
